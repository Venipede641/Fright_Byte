...
draw_set_alpha(0.7);
draw_rectangle(inv_x, inv_y + 10, inv_x + total_width, inv_y + inv_height + 10, false); // Moved down by 20 pixels
draw_set_alpha(1);
...
for (var i = 0; i < slots; i++) {
    ...
    draw_rectangle(slot_x, inv_y + 10, slot_x + slot_width, inv_y + slot_height + 10, true); // Moved down by 20 pixels
    
    // Highlight selected slot... might not use this
    /* if (current_slot.selected) {
        draw_set_color(highlight_color);
        draw_rectangle(slot_x, inv_y + 20, slot_x + slot_width, inv_y + slot_height + 20, true); // Moved down by 20 pixels
    } */
    ...
    draw_sprite_ext(current_slot.sprite, 0, 
                    slot_x + (slot_width / 2), inv_y + (slot_height / 2) + 10,  // Moved down by 20 pixels
                    sprite_scale, sprite_scale, 0, c_white, 1);
    
    // Draw slot number
    draw_set_halign(fa_right);
    draw_set_valign(fa_bottom);
    draw_text(slot_x + (slot_width / 2), inv_y + slot_height - 5 + 10, string(i + 1)); // Moved down by 20 pixels
}
